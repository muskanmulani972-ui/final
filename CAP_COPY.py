...
<td><b>{{ r.status }}</b></td>
<td>{{ r.action_time if r.action_time else "-" }}</td>
...
def warden_summary():
    ...
    for r in rows:
        ...
        # 🔥 FETCH LATEST GUARD ACTION FROM DB
        cursor.execute("""
        ...
        <div id="card_{req_id}" style="display:none;margin-top:10px;width:280px;background:#f0f7ff;border-radius:15px;padding:15px;text-align:center;">
            ...
            <small><b>Guard Action:</b> {guard_status}</small><br>
            <small><b>Guard Action Time (IST):</b> {guard_time}</small>
        </div>
        """
...
def student_return():
    ...
    # Fetch To_Date and In_Time
    cursor.execute("""
        SELECT To_Date, In_Time
        FROM OutpassRequests2
        WHERE Enrollment_No = ?
        ORDER BY id DESC LIMIT 1
    """, (enroll,))
    ...
    if row and row[0] and row[1]:
        ...
        to_date_str = row[0].strip()
        in_time_str = row[1].strip()

        try:
            # Combine To_Date + In_Time into single datetime
            due_datetime = datetime.strptime(
                to_date_str + " " + in_time_str,
                "%Y-%m-%d %H:%M:%S"
            )
        except:
            try:
